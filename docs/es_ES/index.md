...
-   **Panel** : Vous permet d’afficher le panel (Menu
    ...
    caméras (voir plus bas). No olvide activar el panel en la configuración del complemento para acceder más tarde.
...
capture (dénommée "Enregistrement") dans un scénario. En la parte del título pasas las opciones.
...
loin avec des options (voir détail ci-dessous "options avancées des captures"). Dans la partie message, vous n'avez plus qu'à renseigner la commande du plugin (actuellement slack, mail ou transfert) qui fait l’envoi des captures. Puede poner varios separados por &amp;&amp;.
...
> movie=1 sendFirstSnap=1 detectMove=1 ==> envoi la première capture, puis envoi d'une capture à chaque détection de mouvement et enregistre une vidéo jusqu'à la commande "Arrêter Enregistrement" à insérer dans le scénario. La película se almacenará en su Jeedom.
