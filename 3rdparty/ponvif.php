												...
												public function ptz_ContinuousMoveZoom($profileToken,$zoom) {
													...
													*/
													protected function _makeToken() {
														...
														return $this->_passwordDigest($this->username,$this->password,date('Y-m-d\TH:i:s\Z',$timestamp));
													}
													...
													public function events_Subscribe() {
														...
														$address),
														$post_string);
														if ($this->isFault($response=$this->_send_request($this->eventuri,$post_string))) {
															if ($this->intransingent) {
																//throw new Exception('EventsSubscribe: Communication error');
																return false;
															}
														}
														else
														return $response['Envelope']['Body']['CreatePullPointSubscriptionResponse'];
													}
													
													public function events_Pull($endpoint) {
														$REQ=$this->_makeToken();
														$post_string='<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope"><s:Header><Security s:mustUnderstand="1" xmlns="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd"><UsernameToken><Username>%%USERNAME%%</Username><Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest">%%PASSWORD%%</Password><Nonce EncodingType="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary">%%NONCE%%</Nonce><Created xmlns="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">%%CREATED%%</Created></UsernameToken></Security></s:Header><s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema"><PullMessages xmlns="http://www.onvif.org/ver10/events/wsdl"><Timeout>PT1M</Timeout><MessageLimit>100</MessageLimit></PullMessages></s:Body></s:Envelope>';
														$post_string=str_replace(array("%%USERNAME%%",
														"%%PASSWORD%%",
														"%%NONCE%%",
														"%%CREATED%%",
														...
														$address),
														$post_string);
														if ($this->isFault($response=$this->_send_request($endpoint,$post_string))) {
															if ($this->intransingent) {
																//throw new Exception('EventsSubscribe: Communication error');
																return false;
															}
